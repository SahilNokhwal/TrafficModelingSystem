...
public class Sig1JunctionModel extends JunctionModel implements Timed {
	...
	public void generatePaths() {
		...
		System.out.println("generated "+totalNumPaths+ " junction paths for "+
											 "junction with iD "+iD);
		...
		if( (trafficLightArea[1][0].getY() > 100 && trafficLightArea[1][0].getY() < 200) ||
			(trafficLightArea[1][0].getY() > 500 && trafficLightArea[1][0].getY() < 700) )
		{
			trafficLightArea[0][0] = new Rectangle2D.Double( trafficLightArea[0][0].getX(), trafficLightArea[0][0].getY() - 2, 15, trafficLightArea[0][0].getHeight() );
			trafficLightArea[1][0] = new Rectangle2D.Double( trafficLightArea[1][0].getX() + 4, trafficLightArea[1][0].getY() + 2, trafficLightArea[1][0].getWidth(), 15 );
			
			if( (trafficLightArea[1][0].getY() > 500 && trafficLightArea[1][0].getY() < 700) && trafficLightArea[1][0].getX() > 350 )
				trafficLightArea[2][0] = new Rectangle2D.Double( trafficLightArea[2][0].getX()-10, trafficLightArea[2][0].getY() + 2, 15, trafficLightArea[2][0].getHeight() );
			
			trafficLightArea[3][0] = new Rectangle2D.Double( trafficLightArea[3][0].getX() - 4, trafficLightArea[3][0].getY() - 10, trafficLightArea[3][0].getWidth(), 15 );
		}
		else
		{
			if( trafficLightArea[1][0].getX() < 450 )
			{
				trafficLightArea[0][0] = new Rectangle2D.Double( trafficLightArea[0][0].getX(), trafficLightArea[0][0].getY() - 2, 15, trafficLightArea[0][0].getHeight() );
				trafficLightArea[1][0] = new Rectangle2D.Double( trafficLightArea[1][0].getX() + 4, trafficLightArea[1][0].getY() + 2, trafficLightArea[1][0].getWidth(), 15 );
				trafficLightArea[2][0] = new Rectangle2D.Double( trafficLightArea[2][0].getX()-10, trafficLightArea[2][0].getY() + 2, 15, trafficLightArea[2][0].getHeight() );
	...
	public void drawJunctionFeatures(Graphics2D g2d) {
		...
		if( trafficLightArea[1][0].getY() > 300 && trafficLightArea[1][0].getY() < 500 &&
			trafficLightArea[1][0].getX() > 400 )
		{
			for (int i=0;i<maxNumRoads;i++) {
			    for (int j=0;j<ei[i];j++) { //all the lane ends of all sides.
			...
			(trafficLightArea[1][0].getY() > 500 && trafficLightArea[1][0].getY() < 700) )
		{
			...
			g2d.fill( trafficLightArea[1][0] );
			g2d.fill( trafficLightArea[3][0] );
			
			if( (trafficLightArea[1][0].getY() > 500 && trafficLightArea[1][0].getY() < 700) )
			{
				g2d.fill( trafficLightArea[0][0] );
				System.out.println( "~~~" + trafficLightArea[1][0].getX() );
				if( trafficLightArea[1][0].getX() > 350 )
					g2d.fill( trafficLightArea[2][0] );
			}
		}
		else
		{
	...
	public boolean isOKToGo(Car currentcar,double currentdist) {
		...
		if  ( path.cars.size() == 0 && currentcar.inFrontInfo[1] < 15 &&
			( currentcar.inFrontInfo[0]-(currentdist+currentcar.halflength+currentcar.carPath[1].length)) < currentcar.length) return false;
