...
public class CarGenerator implements Timed {
    ...
    private int[] carsPerLanes;
    private int[] generatedCars;
    private int laneNum;
    ...
    public CarGenerator(RoadNetwork roadNetwork,int speed) {
        ...
        carsPerLanes = new int[laneNum];
        generatedCars = new int[laneNum];
...
//        for (int i=0; i < laneNum; i++)
    	...
    	int i = (int)(Math.random()*laneNum);
    	{   		
    		if( generatedCars[i] < carsPerLanes[i] )
    		{
	            lanesCounter[i]++;
	            ...
	            if (startLanes[i].carQueue > 0) {
	                ...
	                if (speedLimited) {
	                    ...
	                    c = new Car(startLanes[i].getStartingLaneSection(),this,ticker,topSpeed+(int)(Math.random()*11.0));
	                }
    ...
    public void setCarsForLanding( int[] arrCarsPerLanding )
    {
    	carsPerLanes = arrCarsPerLanding;
