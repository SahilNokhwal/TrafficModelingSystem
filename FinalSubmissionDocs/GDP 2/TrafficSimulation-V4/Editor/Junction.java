...
public class Junction implements XMLSerializable {
	...
	public float[] getCloneCoords(boolean wantXCoord)  {
	...
	public Junction()  {
		...
		junctionTexture = new TexturePaint(getSignalledJunctionTexture(), new Rectangle2D.Double(0,0,14,14));
		normal = new BasicStroke(1,BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL);
		dottedLine = new BasicStroke(2, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND,1, new float[] {5,5},0);
		thickLine = new BasicStroke(4, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
		centerLine = new BasicStroke(4, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL);
	}
	...
	public void drawHandledJunction(sc scaleManager,Graphics2D g2d) {

		drawSimpleJunction(scaleManager,g2d);
		scaleManager.drawString(String.valueOf(junctionType),x[1],y[1],g2d);

		for (int i=2; i<9;i+=2) {
		    g2d.setColor(Color.black);
		    scaleManager.fill(getHandle(x[i],y[i]),g2d);
		}
	}

	public void drawSimpleJunction(sc scaleManager,Graphics2D g2d) {
		drawSimpleJunction(scaleManager,g2d,x,y,s);
	}

	public void drawSimpleJunction(sc scaleManager,Graphics2D g2d,float[] x,float[] y,Shape s){
...
// disabled end		    
			...
			g2d.setColor(Color.lightGray); 		//kki added
		    Rectangle rtShape = s.getBounds();	//kki added
		    rtShape.setBounds( rtShape.x + 9, rtShape.y + 9, rtShape.width - 18, rtShape.height - 18 );	//kki added
		    Shape sNew = rtShape;
		    scaleManager.fill(sNew,g2d);
	    	g2d.setColor(Color.black);
// disabled by kki
//		    scaleManager.draw(s,g2d);
// disabled end 
		} 
...
//added by kki
		    Rectangle rtShape = s.getBounds();
		    rtShape.setBounds( rtShape.x + 9, rtShape.y + 9, rtShape.width - 18, rtShape.height - 18 );
		    Shape sNew = rtShape;
		    scaleManager.fill(sNew,g2d);
		    ...
		    if( rtShape.x > 220 &&
	    		...
	    		rtShape.y < 180 )
		    {
			    rtShape.setBounds( rtShape.x + 6, rtShape.y + 6, rtShape.width - 12, rtShape.height - 12 );
			    sNew = rtShape;
			    
			    g2d.setColor(Color.yellow);
			    g2d.setStroke(centerLine);
			    scaleManager.drawCircle(sNew, g2d);
		    }
//added end.
		    ...
		    for (int i=2; i<9;i+=2) 
		    {
				if (giveWayPriorities[i] == NO_PRIORITY) {
					if (road[i] == -1) {
						...
						if( (int)x[i-1] == (int)x[(i+1)%8] )
							scaleManager.drawLine((int)x[i-1] + 10,(int)y[i-1], (int)x[(i+1)%8] + 10,(int)y[(i+1)%8],g2d);
						else
							scaleManager.drawLine((int)x[i-1],(int)y[i-1]-10, (int)x[(i+1)%8],(int)y[(i+1)%8]-10,g2d);
					} else {
/* disabled by kki	
						g2d.setStroke(dottedLine);
						scaleManager.drawLine((int)x[i-1],(int)y[i-1],
												 (int)x[(i+1)%8],(int)y[(i+1)%8],g2d);
disabled end*/	
					}
				} else {
					if (x1 == -1) {
...
/* disabled by kki
						...
						scaleManager.drawLine((int)x1,(int)y1,(int)x[i],(int)y[i],g2d);
						g2d.setColor(Color.black);
disabled end*/
					}
				}
		    ...
		    for (int i=2; i<9;i+=2) {
				if (giveWayPriorities[i] != PRIORITY) {
					scaleManager.drawLine((int)x[i-1],(int)y[i-1], (int)x[(i+1)%8],(int)y[(i+1)%8],g2d);
				}
		    }
	...
	public void setShape() {
		...
		jShape.closePath();
		s = jShape;
		originalx = getCloneCoords(true);
		originaly = getCloneCoords(false);
	}
	...
	public void zoomMe(sc scaleManager) {
		for (int i=0;i < 9;++i)
			{
				x[i] = originalx[i];
				y[i] = originaly[i];
		...
		pathShape = GPathUtils.changePoint(pathShape,scaleManager);
		s = pathShape;                            //Update global shape variable
		extractFromPath(pathShape);               //Set point array from shape
	}

	public void afterScroll(sc scaleManager) {
		for (int i=0;i < 9;++i)
			{
				x[i] = originalx[i];
				y[i] = originaly[i];
	...
	public void translate(int direction, int snapSize) {
		float newX = 0, newY = 0;
		switch (direction)
		{
			case RoadDesigner.MOVE_DOWN:
				newY=snapSize;
			break;
			case RoadDesigner.MOVE_UP:
				newY=-snapSize;
			break;
			case RoadDesigner.MOVE_LEFT:
				newX=-snapSize;
			break;
			case RoadDesigner.MOVE_RIGHT:
				newX=snapSize;
			break;
		}
		newX = x[0] + newX;
	...
	private void extractFromPath(GeneralPath path) {
        ...
        public void scaleJunction(float fx, float fy) {
	...
	public BufferedImage getSignalledJunctionTexture() {
		...
		BasicStroke yellowPaint = new BasicStroke(2, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL);
