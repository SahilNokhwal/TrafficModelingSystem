...
public class RoadNetwork {
	...
	public RoadNetwork(RoadDesigner roadDesigner, int speed) {
		...
		while (it.hasNext()) {
		    ...
		    if (tRoad.isSingleLane()) {
				generateRoadLane(tRoad,tlanes[0],null,true,tRoad.busyness);
		    } else {
				for (int i=1;i < Road.maxLaneNum; i++) {
					if (tlanes[i] != null) {
						generateRoadLane(tRoad,tlanes[i],tlanes[0],i%2!=0,tRoad.busyness);
					}
				}
		    }
		}
	 ...
	 */
	private void generateRoadLane(Road parent, GeneralPath path,GeneralPath centerPath,
		...
		int jID1 = roadDesigner.getjunctionID(parent,laneStart);
		int jID2 = roadDesigner.getjunctionID(parent,laneEnd);
	 ...
	 */
	public void drawNetworkComponents(Graphics2D g2d, sc scaleManager) {
		for (int i=0;i < totalLaneNumber; i++) {
	    lanes[i].drawAllCars(g2d,scaleManager);
		}
		for (int i=0;i < totalJunctionNumber; i++) {
	    junctions[i].drawAllCars(g2d,scaleManager);
	...
	public void setCarsForIntersection( int[][] arrCarsForIntersection )
	{
		for( int idx = 0; idx < lanes.length; idx++ )
			lanes[idx].setMaxCountForIntersection( arrCarsForIntersection );
	}
}
