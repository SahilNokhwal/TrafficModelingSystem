...
/**
 * The main class deals with button clicks and menu items. It acts as a mediator
 * between the Road-Designer and the Simulation.
 * */
public class Main extends JFrame implements ActionListener, XMLSerializable {
	...
	JButton changeToDesigner, changeToSim, playPauseButton, graphButton,
			statsButton, minusZoom, plusZoom;
	JSlider framerate;
	JDialog buttonPanel = null;

	JMenuBar menuBar;
	JMenu menu, display, carSpeedsMenuItem, window;
	JMenuItem newMenuItem, saveMenuItem, appletSave, loadMenuItem,
			quitMenuItem;

	JRadioButtonMenuItem[] carSpeedOptions;
	JCheckBoxMenuItem recordData, showData, pathsMenuItem, loadBackDrop,
			communicate;
	JCheckBoxMenuItem commanderItem;

	ImageIcon openIcon, saveIcon, newIcon, quitIcon;
	ImageIcon desIcon, simIcon, selIcon, delIcon, roadIcon, rotateIcon,
			juncIcon;
	...
	private boolean showCommander = true;
	...
	public static void main(String[] args) {
		...
		window.setTitle("Traffic Simulation");
		window.pack();
	}

	/**
	 * The Constructor sets the buttons up and initializes the Road-Editor
	 * panel.
	 */
	public Main() {

		isApplet = false;
		setBounds(200, 1, 640, 480);
		...
		pathsMenuItem = addCheckBoxMenuItem("Show Junction Paths", false,
				display);
		communicate = addCheckBoxMenuItem("Junctions Communicate", false,
				display);
		speedOptionsgroup = new ButtonGroup();
		carSpeedsMenuItem = addCarSpeedOptions("Car speed", speedOptions,
				speedOptionsgroup, display);
		commanderItem = addCheckBoxMenuItem("Commander", true, window);
		...
		load("Saves/cross.xml");
	}

	private void setUpDesignerButtons() {

		buttonPanel = new JDialog(this);
		buttonPanel.setBounds(1, 100, 130, 500);
		buttonPanel.getContentPane().setLayout(gridBag);
		int i = 0;
		...
		minusZoom = addButton("+", 0, i++, 2);
		plusZoom = addButton("-", 0, i++, 2);
		buttonPanel.setVisible(false); // kki
	}

	private void setUpSimButtons() {

		if (buttonPanel == null) {
			buttonPanel = new JDialog();
	 ...
	 *
	 * @param decision
	 *            true if valid to change to sim.
	 */
	...
	public void setInitialWidgetStates() {
		saveMenuItem.setEnabled(isApplet == false);
		appletSave.setEnabled(isApplet == false);
		loadMenuItem.setEnabled(isApplet == false);
		...
		loadBackDrop.setEnabled(isApplet == false);
	...
	public void setMaxCarsCountForLanding(int[] arrCarsForLanding) {
		roadNetwork.carGenerator.setCarsForLanding(arrCarsForLanding);
	}

	public void setMaxCountForIntersection(int[][] arrIntersectionCars) {
		roadNetwork.setCarsForIntersection(arrIntersectionCars);
	}
	...
	private void setControls(int index) {
		...
		if (index == ROADDESIGNER) {
			...
			/*
			 * added by kki if (buttonPanel == null) setUpDesignerButtons();
			 * added end
			 */
			// this.add(buttonPanel,BorderLayout.WEST);

			this.getContentPane().add(designPanel, BorderLayout.CENTER);
			// added by kki buttonPanel.setVisible(true);
			...
			// added by kki drawRoadButton.setSelected(true);
			...
			System.out.println("IN ROADSIM");

			roadNetwork = new RoadNetwork(designPanel, savedSpeed);
			simPanel = new SimPanel(designPanel.getbackdrop(), roadNetwork,
					savedTiming, designPanel.getScaleManager());
			...
			System.out.println("UnIdentified panel index");
	}

	public void StartSimA() {
		simPanel.startSim();
	}

	public void StartSim() {
		if (simPanel != null)
			simPanel.setPauseState(false);
	}

	public void StopSim() {
		if (simPanel != null)
			simPanel.setPauseState(true);
	}
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() == commanderItem) {
			showCommander = commanderItem.getState();
			designPanel.cmdWindow.setVisible(showCommander);
			...
			if (loadBackDrop.getState()) {
				...
				if (returnval == JFileChooser.APPROVE_OPTION) {
					...
					if (currentFile.canRead()) {
						...
						Image backdrop = getToolkit()
								.getImage(backdropFileName);
					...
					System.out
							.println("Load Backdrop command cancelled by user.");
				}
			} else {
				backdropFileName = "null";
				designPanel.backgroundImage = null;
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				String[] args1 = { "NeededFiles/skeleton.txt",
						"AppletSaves/" + file.getName() + ".html", "THEWIDTH",
						"" + designPanel.getSize().width, "THEHEIGHT",
						"" + designPanel.getSize().height, "THESPECFILE",
						"" + file.getName(), "THEARCHIVE",
						file.getName() + ".jar" };

				new Utils.TextRep(args1); // A text search and replace class.

				String[] args2 = { "NeededFiles/AppletJarrer.sh",
						"AppletSaves/" + file.getName() + ".sh", "THEJARNAME",
						file.getName() + ".jar", "THESPECFILE",
						"" + file.getName() };

				new Utils.TextRep(args2); // A text search and replace class.

				String[] args3 = { "NeededFiles/AppletJarrer.bat",
						"AppletSaves/" + file.getName() + ".bat", "THEJARNAME",
						file.getName() + ".jar", "THESPECFILE",
						"" + file.getName() };
	...
	public void loadSelf(XMLElement element) {
		...
		backdropFileName = element.getAttribute("BackDropFile")
				.getStringValue();
	...
	private void addButtonFinal(JButton button, int x, int y, int w) {
		...
		buttonPanel.getContentPane().add(button);
	}

	private JButton addButton(ImageIcon icon, int x, int y, int w) {
		JButton button = new JButton(icon);
		addButtonFinal(button, x, y, w);
		return button;
	}

	private JButton addButton(String text, int x, int y, int w) {
	...
	private JSlider addSlider(int rule, int start, int end, int value, int x,
			int y, int w) {
		...
		buttonPanel.getContentPane().add(slider);
		return slider;
	}
	...
	private void addLabel(ImageIcon icon, int x, int y, int w) {
		...
		buttonPanel.getContentPane().add(l);
	}
	...
	private void addSeperator(int x, int y) {
		...
		buttonPanel.getContentPane().add(c);
	}
	...
	private JCheckBoxMenuItem addCheckBoxMenuItem(String message,
			boolean state, JMenu menu) {
	...
	private JMenu addCarSpeedOptions(String message, int ops,
		...
		for (int i = 0; i < ops; i++) {
			...
			if (savedSpeed == i)
				carSpeedOptions[i].setSelected(true);
