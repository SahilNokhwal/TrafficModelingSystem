...
public abstract class GPathUtils {
	...
	/*
	 * try to do points tranlsation
	 */
	public static GeneralPath scalePath(GeneralPath path, float factor) {
		GeneralPath newPath = new GeneralPath(GeneralPath.WIND_EVEN_ODD);
	...
	public static GeneralPath movePath(GeneralPath path, int qx, int qy) {
	...
	public static GeneralPath mergePaths(GeneralPath p1, GeneralPath p2) {
		double seg[] = new double[6];
		PathIterator i = p2.getPathIterator(null);
		i.next();
		while (!i.isDone()) {
			int segType = i.currentSegment(seg);
			p1.lineTo((int) seg[0], (int) seg[1]);
	...
	/**
	 * returns a path equal to "path" but with the path point "pointToChange" at
	 * location of "newPoint".
	 */
	...
	public static GeneralPath getPathFromIndex(int point, GeneralPath path, boolean stop) {
	...
	public static int getAverageDirection(GeneralPath path) {
		...
		return GeoUtils.calculateGeneralDirection((int) seg[0], (int) seg[1],
				(int) path.getCurrentPoint().getX(), (int) path
	...
	public static double getAverageAngle(GeneralPath path) {
		...
		return GeoUtils.getAngle(seg[0], seg[1], path.getCurrentPoint().getX(),
	...
	public static GeneralPath changePoint(GeneralPath source, sc scaleManager) {
		GeneralPath newPath = new GeneralPath(GeneralPath.WIND_EVEN_ODD);
		boolean seen = false;
		Point2D newPoint = null;

		double seg[] = new double[6];
		PathIterator pi = source.getPathIterator(null);

		while (!pi.isDone()) {
			int segType = pi.currentSegment(seg);
			newPoint = scaleManager.real_to_screen(seg[0],seg[1]);
			switch (segType) {
			case PathIterator.SEG_MOVETO:
				newPath.moveTo((int) newPoint.getX(), (int) newPoint.getY());
				break;
	...
	public static void dumpPath(GeneralPath l0,Graphics2D g2d) {
		double seg[] = new double[6];
		int index = 0;
		PathIterator pi = l0.getPathIterator(null);
		...
		while (!pi.isDone()) {
			...
			case PathIterator.SEG_MOVETO:
				...
				g2d.drawOval((int) seg[0], (int) seg[1], 10, 10);
				g2d.drawString(Integer.toString(index),(int) seg[0], (int) seg[1]);
				break;
			case PathIterator.SEG_LINETO:
				g2d.setColor(Color.RED);
				g2d.drawOval((int) seg[0], (int) seg[1], 18, 18);
				g2d.drawString(Integer.toString(index),(int) seg[0], (int) seg[1]);
				break;
			}
			pi.next();
