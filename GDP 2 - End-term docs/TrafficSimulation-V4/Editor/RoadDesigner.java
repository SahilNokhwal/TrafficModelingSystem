...
public class RoadDesigner extends JPanel
	implements MouseListener, MouseMotionListener, 
						 ComponentListener, XMLSerializable, AdjustmentListener {
	...
	public final static int MOVE_UP = 17;
	public final static int MOVE_DOWN = 18;
	public final static int MOVE_RIGHT = 19;
	public final static int MOVE_LEFT = 20;
	...
	private Road currentRoadSection;
	...
	private int snapSize = 8;
	private float scaleFactor = 0;
	private final float scaleStep = 0.3f;

	private PopMenuHandle popupMenu;

	private Point2D saveCurrentHandle;
	private Road saveCurrentRoad;
	public static Commander cmdWindow;
	private sc scaleManager;
	private int xViewSize = 800, yViewSize = 600;
	private double zoom_factor = 1.25f;
	private double zoom_scale = 0.0f;
	private JScrollBar jScrollHoriz;
	private JScrollBar jScrollVert;
	private int oldScrollX = 0, oldScrollY = 0;
	private InfoSection currentRoadSectionInfo;
	private int currentRoadSectionIndex = -1;
	private Road currentRoadSelection = null;
	/*
	...
	*/
	...
	public RoadDesigner(Main parent) {
		...
		if (cmdWindow == null) {
			cmdWindow = new Commander(parent);
			cmdWindow.setLocation(400,400);
			cmdWindow.setVisible(true);	//added by kki
		}
		...
		jScrollHoriz = new JScrollBar(JScrollBar.HORIZONTAL);
		jScrollHoriz.setMaximum(500);
		add(jScrollHoriz,BorderLayout.SOUTH);
		jScrollVert = new JScrollBar(JScrollBar.VERTICAL);
		add(jScrollVert,BorderLayout.EAST);

		jScrollHoriz.addAdjustmentListener(this);
		jScrollVert.addAdjustmentListener(this);
	...
	private void setGraphics () {
		...
		backBufferG2D.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON);
	...
	public void currentHandleAsStartCars()
	{
	...
	public void adjustmentValueChanged(AdjustmentEvent e) {
		if(e.getSource() == jScrollHoriz)
		{		
			scaleManager.scrollBars_Scroll(jScrollHoriz,oldScrollX,jScrollHoriz.getValue(),true);
			oldScrollX = jScrollHoriz.getValue();
		}
		else
		{
			scaleManager.scrollBars_Scroll(jScrollVert,oldScrollY,jScrollVert.getValue(),false);
			oldScrollY = jScrollVert.getValue();
		}
		setBackBuffer(backBufferG2D);
	...
	public void upZoomRoad()
	{
		scaleManager.zoom(1/zoom_factor);
		applyZoom();
		zoom_scale+=1/zoom_factor;
	}

	public void downZoomRoad()
	{
		scaleManager.zoom(zoom_factor);
		applyZoom();
		zoom_scale-=1/zoom_factor;
	}
	...
	public boolean currentRoadAsStart()
	{
		if (saveCurrentRoad != null)
		{
			Point2D handle = saveCurrentHandle;
			boolean isStartHandle = false, isEndHandle = false;
			Point2D startHandle = saveCurrentRoad.getStartHandle();
			Point2D endHandle = saveCurrentRoad.getEndHandle();
			isStartHandle = handle.equals(startHandle);
			isEndHandle = handle.equals(endHandle);
			if (isEndHandle)
			{
				alertBox.showMessage(parent,"suka caption","suka text");
				return false;
			}
			saveCurrentRoad.setCarGen(isStartHandle,isEndHandle);
		    setBackBuffer(backBufferG2D);
		    repaint();
		}
		return true;
	...
	{
		if (currentRoadSection == null) return;
		Line2D line = currentRoadSection.getHandleAsLine();
		double angle = GeoUtils.getAngle(line.getX1(),line.getY1(),line.getX2(),line.getY2());
		Point2D pointToChange = currentRoadSection.getEndHandle();
		Point2D p = currentRoadSection.getStartHandle();
		double x = GeoUtils.getXPointAtAngle(p.getX(),angle,cmdWindow.getCarCount());
		double y = GeoUtils.getYPointAtAngle(p.getY(),angle,cmdWindow.getCarCount());
		currentRoad.modifyRoadHandle(pointToChange,new Point2D.Double(x,y));
		testToJoinRoads();
	...
	public void mousePressed(MouseEvent e) {
		...
		getcurrentRoadSection();
		if(currentRoadSection != null)
		{
			Road tempRoad = currentRoadSection;
			cmdWindow.setCarCount(tempRoad.length());
			cmdWindow.setRoadName(currentRoadSectionInfo.Name);
		}
		//if (e.getButton() == MouseEvent.BUTTON3) { //Java 1.4
		if ((e.getModifiers() & e.BUTTON3_MASK)== e.BUTTON3_MASK) {
		if (currentHandle != null)
		{
			saveCurrentHandle = currentHandle;
			if (currentRoad != null)
				saveCurrentRoad = currentRoad;
			popupMenu = new PopMenuHandle(this,(int)currentXYPoint.getX(),(int)currentXYPoint.getY());
		}
	...
	public void mouseDragged(MouseEvent e) {
	    ...
	    if (drawMode == CLICKED_ON_HANDLE || drawMode == DRAG_ROAD_HANDLE){
				...
				//distance /= currentRoad.laneWidth;
	...
	public void paint(Graphics g) {
	    ...
	    extendedRoad.drawRoad(offScreenG2D,scaleManager);
	    extendedRoad.calculateShape();
	    drawRoadIntersections(offScreenG2D,extendedRoad);
	    extendedRoad.drawDirectedMidPoints(offScreenG2D, scaleManager);
	    extendedRoad.drawHandles(offScreenG2D,scaleManager);
	    ...
	    currentRoad.drawRoad(offScreenG2D,scaleManager);
	    currentRoad.calculateShape();
	    currentRoad.drawDirectLengths(offScreenG2D,scaleManager);
	    ...
	    if (currentRoad != null) currentRoad.drawRoad(offScreenG2D,scaleManager);
	    ...
	    currentRoad.calculateShape();
	    drawRoadIntersections(offScreenG2D,currentRoad);
	    currentRoad.drawDirectedMidPoints(offScreenG2D,scaleManager);
	    currentRoad.drawHandles(offScreenG2D,scaleManager);
	    ...
	    currentJunction.drawHandledJunction(scaleManager,offScreenG2D);
		}
	    ...
	    scaleManager.fill(getSPointRectangle((int)currentHandle.getX(), (int)currentHandle.getY()),offScreenG2D);
		} else if (currentMidPoint != null) {
	    offScreenG2D.setColor(Color.red);
	    scaleManager.fill(getSPointRectangle((int)currentMidPoint.getX(), (int)currentMidPoint.getY()),offScreenG2D);
		} else if (currentRoadSide != null) {
	    offScreenG2D.setColor(Color.red);
	    scaleManager.fill(currentRoadSide,offScreenG2D);
		}
	...
	private void lookForSection(Graphics2D g2d) {
		final Iterator it = roads.iterator();
		while (it.hasNext()) {
			final Road tempRoad = (Road)it.next();
			//watchSection(tempRoad, g2d);
		}	
/*		if(currentRoad != null) {
			watchSection(currentRoad, g2d);
		}*/
	}
	...
	private void snapTo(MouseEvent e) {
		Point2D newPoint = scaleManager.screen_to_real(e.getX(),e.getY());
		x = (int)newPoint.getX();
		y = (int)newPoint.getY();
		...
		currentSnapPoint = new Point2D.Float(snapX,snapY);
	}
    ...
    private void drawAllOldRoads(Graphics2D g2d) {
			...
			while (it.hasNext()) {
				final Road tempRoad = (Road)it.next();
				tempRoad.drawRoad(g2d,scaleManager);
			}
    }
	...
	private void drawAllRoadDirectedMidPoints(Graphics2D g2d) {
		...
		while (it.hasNext()) {
	    final Road tempRoad = (Road)it.next();
	    tempRoad.drawDirectedMidPoints(g2d,scaleManager);
		}
	}
	...
	private void drawAllRoadHandles(Graphics2D g2d){
		...
		while (it.hasNext()) {
	    final Road tempRoad = (Road)it.next();
	    tempRoad.drawHandles(g2d,scaleManager);
		}
	}
	...
	private void drawAllOldJunctions(Graphics2D g2d) {
	    ...
	    tempJunc.drawHandledJunction(scaleManager,g2d);
		}
	}
	...
	private void drawRoadIntersections(Graphics2D g2d, Road road){
	    ...
	    if (tempRoad.getNumberOfSections() >= 1 && tempRoad != road) {
				...
				scaleManager.fill(intersectArea,g2d);
				//g2d.fill(intersectArea); +++
				g2d.setStroke(normal);g2d.setColor(Color.red);
				scaleManager.draw(intersectArea,g2d);
	...
	private boolean getcurrentRoadSection() {
		currentRoadSection = null;
		if (currentRoad != null) {
			currentRoadSection = currentRoad.getSection(x,y);
			currentRoadSectionInfo = currentRoad.getSectionInfo(x,y);
			currentRoadSelection = currentRoad;
			return true;
		}
		final Iterator it = roads.iterator();
		int index = 0;
		while (it.hasNext()) {
			final Road tempRoad = (Road)it.next();
			currentRoadSection = tempRoad.getSection(x,y);
			if (currentRoadSection != null) {
				currentRoadSelection = tempRoad;
				currentRoadSectionInfo = tempRoad.getSectionInfo(x,y);
				return true;
			}
			++index;
		}
	...
	private void placeJunctionInRoads() {
		...
		if (index2 != -1) {
	    Road newRoad = currentRoad.getSectionFrom(roadIndex++,index2,false);
	    roads.add(newRoad);
	    ...
	    if (index2 != -1) {
				Road newRoad = oRoad.getSectionFrom(roadIndex++,index2,false);
				roads.add(newRoad);
				currentJunction.attachRoad(newRoad.getID(),dir2,true);
				currentJunction.updateJunctionLook();
				newRoad.setRoadShapeVariables();
	...
	private boolean testForValidity() {
		...
		if (currentRoad != null && !testLaneForValidity(currentRoad.getLane(0)))
			return false;

		final Iterator it = roads.iterator();
		while (it.hasNext()) {
			final Road tempRoad = (Road) it.next();
			if (!testLaneForValidity(tempRoad.getLane(0)))
				return false;
		}
		return true;
	}
	...
	private boolean testPointForValidity(Point2D p) {
		...
		if(currentRoad.startCarGen) return true;
	...
	public void drawAllNeatenedRoads(Graphics2D offgraphics) {
	    ...
	    tempRoad.drawRoad(offgraphics,scaleManager);
		}
	}
	...
	public int getjunctionID(Road parent, Point2D p) {
	...
	public Image getbackdrop() {
		...
		while (it.hasNext()) {
	    final Junction tempJunc = (Junction)it.next();
	    tempJunc.drawSimpleJunction(scaleManager,offgraphics);
		}
	...
	public void loadSelf(XMLElement element) {
		...
		firstDrawTime = true;
		Point2D ptMax = getMaxSize();
		scaleManager.set_parameters(0,ptMax.getX(),0,ptMax.getY(),xViewSize,yViewSize);
	}
	...
	private Point2D getMaxSize() {
		Iterator it = roads.iterator();
		double maxx = 0,maxy = 0;
		Point2D posHandle;
		while (it.hasNext()) {
			final Road tempRoad = (Road) it.next();
			posHandle = tempRoad.getExternalHandle();
			maxx = Math.max(maxx,posHandle.getX());
			maxy = Math.max(maxy,posHandle.getY());
		}
		return new Point2D.Double(maxx,maxy);
	}
	
	public void updateSectionName(String newName) {
		currentRoadSelection.setSectionName(currentRoadSectionInfo.index,newName);
	}
