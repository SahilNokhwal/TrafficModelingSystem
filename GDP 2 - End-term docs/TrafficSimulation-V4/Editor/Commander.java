...
public class Commander extends JDialog {
	...
	private void initialize() {
		...
		for( int i = 0; i < 8; i++ ) arrGeneratedCars[i] = 25;
			txtCarCount.setText(String.valueOf(arrGeneratedCars[0]));
			
		for( int i = 0; i < 6; i++ ) for( int j = 0; j < 4; j++ )
			arrIntersectionCars[i][j] = 60;
		
		txtIntsectionCarCount.setText(String.valueOf(arrIntersectionCars[0][0]));
	}
	/**
	...
	private JPanel getJPanel1() {
		if (jPanel1 == null) {
			...
			jCmbIntersection.addActionListener(new java.awt.event.ActionListener() 
			{ 
				public void actionPerformed(java.awt.event.ActionEvent e) 
				{
					txtIntsectionCarCount.setEnabled(true);
					String strCurItem = (String) jCmbDirection.getSelectedItem();
					
					if( jCmbIntersection.getSelectedIndex() == 2 && strCurItem.equals("East") )
					{
					...
					if( jCmbIntersection.getSelectedIndex() == 4 && strCurItem.equals("South") )
					{
						txtIntsectionCarCount.setText("");
						txtIntsectionCarCount.setEnabled(false);
						return;
					...
					if( strCurItem.equals("East") )
						setIntersectionCarCount(arrIntersectionCars[jCmbIntersection.getSelectedIndex()][0]);
					else if( strCurItem.equals("West") )
						setIntersectionCarCount(arrIntersectionCars[jCmbIntersection.getSelectedIndex()][1]);
					else if( strCurItem.equals("North") )
						setIntersectionCarCount(arrIntersectionCars[jCmbIntersection.getSelectedIndex()][2]);
					else if( strCurItem.equals("South") )
						setIntersectionCarCount(arrIntersectionCars[jCmbIntersection.getSelectedIndex()][3]);
				}
			});
			...
			jCmbDirection.addActionListener(new java.awt.event.ActionListener() 
			{ 
				public void actionPerformed(java.awt.event.ActionEvent e) 
				{
					txtIntsectionCarCount.setEnabled(true);
		...
		if (jButton == null) {
			...
			{ 
				public void actionPerformed(java.awt.event.ActionEvent e) 
				{
					...
					{
						JOptionPane.showMessageDialog(new JFrame(), "Please input number for car count.");
						return;
					}
					
					arrGeneratedCars[jCmbLanding.getSelectedIndex()] = Integer.valueOf(txtCarCount.getText());
				}
			});
		}
		return jButton;
		...
		if (jButton2 == null) {
			...
			{ 
				public void actionPerformed(java.awt.event.ActionEvent e) 
				{
					...
					{
						JOptionPane.showMessageDialog(new JFrame(), "Please input car count for intersection.");
						return;
					}
					
					if( txtIntsectionCarCount.isEnabled() )
					{
						arrIntersectionCars[jCmbIntersection.getSelectedIndex()][jCmbDirection.getSelectedIndex()] = Integer.valueOf(txtIntsectionCarCount.getText());
					}
				}
			});
		}
